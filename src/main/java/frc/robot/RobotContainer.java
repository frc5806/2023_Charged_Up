...
public class RobotContainer {
  ...
  // Commands
  /////////////////////////////////////////////////////////////////////////////////////////////////////////
  // private final Command startArm = Commands.runOnce(armExtension::enable,
  // armExtension);
  // private final Command stopArm = Commands.runOnce(armExtension::disable,
  // armExtension);
  ...
  public RobotContainer() {
    ...
    driveTrain.setDefaultCommand(
        new RunCommand(
            () -> driveTrain.arcadeDrive(
                -joystick1.getRawAxis(1)*0.8, joystick1.getRawAxis(0)*0.8, true),
            driveTrain));
  ...
  private void configureButtonBindings() {
    // Joystick
    new JoystickButton(joystick1, 2).whileTrue(intake.runIntake(0.5));
    new JoystickButton(joystick1, 1).whileTrue(intake.runIntake(-0.5));

    // new JoystickButton(joystick1, 5).onTrue(pneumatic.reverseIntake());
    new JoystickButton(joystick1, 5).onTrue(new InstantCommand(() -> pneumatic.changeIntakeMode(), intake));
    // new JoystickButton(joystick1, 5).startEnd(new InstantCommand(() ->
    // pneumatic.changeIntakeMode(), ));

    // new JoystickButton(joystick1, 7).whileTrue(intake.adjustAngle(0.2));
    // new JoystickButton(joystick1, 8).whileTrue(intake.adjustAngle(-0.2));
  ...
  public void showTelemetry() {
    ...
    // System.out.println(networkTable.getEntry("tid").getDoubleArray(new
    // double[6])[0]);
    ...
    // Double tagID =
    // NetworkTableInstance.getDefault().getTable("limelight").getEntry("tid").getDoubleArray(new
    // double[6]);
    // System.out.println();
  ...
  public Command getAutonomousCommand() {
    ...
    return driveTrain.stupidAutoDriveForwardOutake1Command(driveTrain, intake, 0.05 );

    // return driveTrain.stupidAutoArmBackwards(driveTrain, intake, 0.2);

    // return
    // arm.rotateArm().andThen(claw.runClaw(0.2)).andThen(driveTrain.stupidAutoDriveBackwardsCommand(driveTrain,
    // -0.2));
  }
}